  ...
  async exportToExcel(filename = 'AMC_Schedule') {
    ...
    if (paymentData.length > 0) {
      ...
      paymentData.forEach(row => {
        ...
        const statusColIndex = Object.keys(paymentData[0]).indexOf('Status');
        const dateColIndex = Object.keys(paymentData[0]).indexOf('Payment Date');
    ...
    if (quarterData.length > 0) {
      ...
      quarterData.forEach(row => {
        ...
        const statusColIndex = Object.keys(quarterData[0]).indexOf('Payment Status');
        const dateColIndex = Object.keys(quarterData[0]).indexOf('Payment Date');
        ...
        if (dateColIndex !== -1 && row['Payment Date']) {
          newRow.getCell(dateColIndex + 1).fill = {
            type: 'pattern',
            pattern: 'solid',
...
export class WarrantyExportManager {
  ...
  async exportToExcel(filename = 'Warranty_Tracking') {
    ...
    if (paymentData.length > 0) {
      ...
      paymentData.forEach(row => {
        const newRow = paymentWS.addRow(Object.values(row));
        const statusColIndex = Object.keys(paymentData[0]).indexOf('Payment Status');
        const dateColIndex = Object.keys(paymentData[0]).indexOf('Payment Date');
  ...
  exportToCSV(filename = 'Warranty_Tracking') {
    const scheduleData = this.prepareScheduleData();
    const keys = Object.keys(scheduleData[0]);
    const csvRows = [
      keys.join(','),
  ...
  // PDF report export
  exportToPDF(filename = 'Warranty_Tracking_Report') {
    const doc = new jsPDF();
    doc.setFontSize(16);
